#!/usr/bin/env bash
# shikki-board.sh — Launch the Shikki orchestrator tmux layout
#
# v2: Dynamic dispatch. Board tab starts empty — HeartbeatLoop fills it.
#
# Layout:
#   Tab 1 (orchestrator): shikki heartbeat loop — dispatches tasks dynamically
...
#
# Usage:
#   bash scripts/shikki-board.sh           # create layout
#   bash scripts/shikki-board.sh --attach  # just attach if session exists
...
if ! curl -sf http://localhost:3900/health >/dev/null 2>&1; then
  echo "Warning: Shikki backend not reachable at localhost:3900"
  echo "Start it with: docker compose up -d"
fi

echo -e "${GREEN}Launching Shikki Board${RST}"
...
echo -e "${GREEN}Shikki Board ready${RST}"
