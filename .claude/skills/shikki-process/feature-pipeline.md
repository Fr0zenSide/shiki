...
After each phase, sync to Shiki if available: Use the `shikki_save_event` MCP tool with:
```json
{
...
**After each phase**: Record a checkpoint: Use the `shikki_save_event` MCP tool with:
```json
...
**On failure**: Record with `"status":"failed"`, then check routing: Use the `shikki_save_event` MCP tool with:
```json
{ "type": "pipeline_route", "scope": "md-feature", "data": { "runId": "<RUN_ID>", "failedPhase": "<phase>" } }
```

**On completion**: Use the `shikki_save_event` MCP tool with: `{ "type": "pipeline_completed", "scope": "md-feature", "data": { "runId": "<RUN_ID>", "status": "completed" } }`
