...
**After each gate**: Record a checkpoint: Use the `shikki_save_event` MCP tool with:
```json
...
**On gate failure**: Record with `"status":"failed"`, then check routing rules: Use the `shikki_save_event` MCP tool with:
```json
...
**On completion**: Use the `shikki_save_event` MCP tool with: `{ "type": "pipeline_completed", "scope": "pre-pr", "data": { "runId": "<RUN_ID>", "status": "completed" } }`
