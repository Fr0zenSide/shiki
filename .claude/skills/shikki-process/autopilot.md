...
1. **Create decision in queue**: Use the `shikki_save_event` MCP tool with:
   ```json
   {
...
2. **Block the task**: Use the `shikki_save_event` MCP tool with:
   ```json
   {
     "type": "task_blocked",
     "scope": "orchestrator",
...
Record to shikki DB: Use the `shikki_save_event` MCP tool with:
```json
{
...
Record autopilot events to shikki DB throughout:
...
| Context compaction during autopilot | Save state to shikki DB, resume from last wave |
