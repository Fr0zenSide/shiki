...
moto_get_context(project="example/brainy", scope="protocols")
moto_get_type(project="example/brainy", name="Article")
moto_get_pattern(project="example/brainy", name="error_pattern")
```

> **MCP namespace rule**: All Moto tools use `moto_` prefix. Shikki's internal
> knowledge tools use `shiki_` prefix. No collision — different data sources,
> different query semantics (Moto = static pre-built cache, Shikki = live events).
> Federation: `shiki_get_type("X")` may delegate to `moto_get_type(project, "X")`
> when the type is not in ShikiDB but exists in the project's Moto cache.
...
**Moto** (元, origin/source — @Daimyo decision) — an open standard for pre-computed project architecture caches. A `.moto` dotfile in a repo points AI agents to a cached representation of the project's architecture (protocols, types, dependencies, patterns, tests) so they never need to scrape the raw repo.
...
3. **GitHub Action** — computes cache at push/release, publishes to GitHub Pages or Moto CDN
4. **Go core** — Tree-sitter-based multi-language parser, generates cache + typed client wrappers
5. **MCP interface** — standard query tools (`moto_get_context`, `moto_get_type`, etc.)
...
**Moto** (元) — selected by @Daimyo. "Origin/source" in Japanese. `.moto` dotfile. Badge: `[moto cached | v1.2.0]`. CLI: `moto build`, `moto serve`, `moto publish`.
...
### Shikki Integration Layer

How ShikkiCore discovers and consumes `.hanko` files:

1. **Discovery**: On project init (`shikki start` or `shikki heartbeat`), ShikkiCore checks for `.moto` at repo root. If present, parse the TOML and register the cache endpoint.

2. **Cache source precedence**:
   - **Solo dev**: Local ingestion (via `ProjectAnalyzer`) is primary, `.moto` cache is a fast-start fallback when local ingestion hasn't run yet
   - **Team mode** (multi-user ShikkiDB): Moto CDN cache is primary (shared, CI-fresh). Local ingestion overrides only if newer (compare `commit` hash)
   - **No `.moto` file**: Local ingestion only (current behavior, unchanged)

3. **CodeGen Engine integration**: Wave 1 of CodeGen (`shikki-codegen-engine.md`) defines 6 MCP tools (`get_project_context`, `get_protocol`, `get_type`, `get_pattern`, `get_dependency_graph`, `suggest_implementation`). These tools query the local `ArchitectureCache`. When a `.moto` file is present, the cache can be pre-seeded from the Moto endpoint instead of re-parsing source files — same schema, different source.

4. **Answer Engine integration**: `ArchitectureLookup` in the Answer Engine (`shikki-answer-engine.md`) already delegates to `ArchitectureCache`. No change needed — Moto feeds the same cache struct.

5. **MCP federation**: `shiki_get_type("SomeType")` checks ShikiDB first. If not found and a Moto cache is registered, transparently delegates to `moto_get_type(project, "SomeType")`. This is optional in v1, required in v2 for multi-project workspaces.

### Open Questions for @Daimyo

1. **Name**: ~~Hanko, Kamon, or something else?~~ → **Moto** (元) — decided by @Daimyo.
2. **Cache hosting**: GitHub Pages (free, decentralized) vs. Moto CDN (centralized, MCP-native, revenue opportunity)?
