...
/// - Normalizes status to valid `SpecLifecycle` enum values
...
public struct SpecMigrationService: Sendable {
    ...
    func migrateMarkdownStyle(content: String, filePath: String) -> MigrationResult {
        ...
        // Keep the original body intact (including the heading and inline meta — don't strip them)
        var result = yaml
    ...
    func parseInlineMetaField(_ line: String) -> (key: String, value: String)? {
        ...
        // e.g., "Created: 2026-03-23 | Author: @Sensei | Status: draft"
        // We return just the first field; the caller iterates over pipe-split segments separately
    ...
    /// Map common status strings to valid `SpecLifecycle` raw values.
    public func normalizeStatus(_ raw: String) -> String {
        ...
        // Direct matches
        if SpecLifecycle(rawValue: lowered) != nil {
            return lowered
        }
        ...
        default:
            ...
            if let lifecycle = SpecLifecycle(rawValue: firstWord) {
    ...
    func extractFirstSentence(_ text: String) -> String {
        ...
        for (i, char) in chars.enumerated() {
            result.append(char)
            if char == "." || char == "!" || char == "?" {
                // Check if next char is space, end, or a quote
        ...
        // Strip any leading markdown like `**` or `-`
