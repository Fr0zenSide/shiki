...
public struct SpecFrontmatterParser: Sendable {
    ...
    public func parse(content: String) throws -> SpecMetadata {
        ...
        guard let status = SpecLifecycle(rawValue: statusRaw) else {
    ...
    func parseReviewers(_ yaml: String) throws -> [ReviewerEntry] {
    ...
    /// Build a ``ReviewerEntry`` from parsed key-value pairs.
    private func buildReviewerEntry(
        _ fields: [String: String],
        sectionsValidated: [Int],
        sectionsRework: [Int]
    ) throws -> ReviewerEntry? {
        ...
        guard let verdict = ReviewerVerdict(rawValue: verdictRaw) else {
            return ReviewerEntry(who: who, verdict: .pending)
        }
    ...
    /// Parse the `flsh:` nested block.
    func parseFlshBlock(_ yaml: String) -> FlshBlock? {
        let lines = yaml.components(separatedBy: "\n")
        var inFlsh = false
        var summary: String?
        var duration: String?
        ...
        if summary == nil && duration == nil && sections == nil { return nil }
        return FlshBlock(summary: summary, duration: duration, sections: sections)
    }
