...
public protocol EventTransport: Sendable {
    ...
    func subscribe(to channel: String) -> AsyncStream<ShikkiEvent>
...
public actor WebSocketEventTransport: EventTransport {
    ...
    /// Parse a raw JSON message into a ShikkiEvent, optionally filtering by channel.
    /// The channel filter matches against the event scope's project slug or event type.
    nonisolated static func parseEvent(from data: Data, channel: String) -> ShikkiEvent? {
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601

        guard let event = try? decoder.decode(ShikkiEvent.self, from: data) else {
            return nil
        }
...
public actor MockEventTransport: EventTransport {
    private var continuations: [AsyncStream<ShikkiEvent>.Continuation] = []
    ...
    public func emit(_ event: ShikkiEvent) {
        for c in continuations {
            c.yield(event)
        }
    }

    private func addContinuation(_ continuation: AsyncStream<ShikkiEvent>.Continuation) {
