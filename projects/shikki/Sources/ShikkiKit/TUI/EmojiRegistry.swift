...
public enum EmojiRegistry: Sendable {
    ...
    public static func generateShellAliases() -> String {
        var lines: [String] = [
            "# Shikkimoji shell aliases — generated by shi",
            "# Add to ~/.zshrc or ~/.bashrc: eval \"$(shi doctor --emit-aliases)\"",
            "",
        ]
        ...
        for entry in all where !seenCommands.contains(entry.command) {
            ...
            lines.append("alias sk-\(entry.command)='shi \(entry.command)'")
        }
        ...
        for entry in all {
            lines.append("alias 'sk-\(entry.emoji)'='shi \(entry.command)'")
        }
