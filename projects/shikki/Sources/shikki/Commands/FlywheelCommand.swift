    ...
    struct TelemetrySubcommand: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            if let level {
                ...
                _ = try store.setLevel(telemetryLevel)
                let emoji: String
    ...
    struct RiskSubcommand: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            for filePath in files {
                ...
                let levelColor = colorForTier(profile.tier)
                print("\(levelColor)\(profile.tier.rawValue.uppercased())\u{1B}[0m [\(String(format: "%.0f%%", profile.score * 100))] \(filePath)")

                if verbose {
                    for signal in profile.signals {
                        let sign = signal.contribution >= 0 ? "+" : ""
                        print("  \u{1B}[2m\(signal.name): \(sign)\(String(format: "%.2f", signal.contribution)) (weight=\(String(format: "%.2f", signal.weight)))\u{1B}[0m")
                    }
                }
            }
        }
...
extension FlywheelCommand {
    struct StatusSubcommand: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            print("  Risk weights: default (churn=20%, coverage=20%, complexity=25%)")
        }
    }
}
        ...
        func run() async throws {
            ...
            if report.metrics.isEmpty {
                ...
                for metric in report.metrics {
                    let percentileStr = metric.percentile.map { "p\(String(format: "%.0f", $0))" } ?? "-"
                    print("  \(metric.name): \(String(format: "%.2f", metric.value)) \(metric.unit) [\(percentileStr)]")
                }
            }
