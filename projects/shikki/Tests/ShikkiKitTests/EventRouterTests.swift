...
struct EventClassificationTests {
    ...
    func sessionStartIsProgress() {
        let event = ShikkiEvent(source: .orchestrator, type: .sessionStart, scope: .session(id: "s1"))
        let significance = EventClassifier.classify(event)
    ...
    func decisionIsPriority() {
        let event = ShikkiEvent(source: .orchestrator, type: .decisionPending, scope: .project(slug: "maya"))
        let significance = EventClassifier.classify(event)
    ...
    func budgetIsAlert() {
        let event = ShikkiEvent(source: .orchestrator, type: .budgetExhausted, scope: .project(slug: "maya"))
        let significance = EventClassifier.classify(event)
    ...
    func redFlagIsCritical() {
        let event = ShikkiEvent(source: .system, type: .custom("redFlag"), scope: .global)
        let significance = EventClassifier.classify(event)
...
struct EventEnrichmentTests {
    ...
    func enrichWithSessionState() async {
        ...
        let event = ShikkiEvent(source: .agent(id: "maya:task", name: nil), type: .codeChange, scope: .session(id: "maya:task"))
    ...
    func enrichWithCompanySlug() async {
        ...
        let event = ShikkiEvent(source: .orchestrator, type: .heartbeat, scope: .project(slug: "wabisabi"))
        let context = await enricher.enrich(event)
...
struct PatternDetectionTests {
    ...
    func stuckAgentDetected() async {
        ...
        for _ in 0..<3 {
            await detector.record(ShikkiEvent(source: .orchestrator, type: .heartbeat, scope: sessionScope))
        }
    ...
    func noStuckWithProgress() async {
        ...
        await detector.record(ShikkiEvent(source: .orchestrator, type: .heartbeat, scope: scope))
        await detector.record(ShikkiEvent(source: .agent(id: "maya:task", name: nil), type: .codeChange, scope: scope))
        await detector.record(ShikkiEvent(source: .orchestrator, type: .heartbeat, scope: scope))
    ...
    func repeatFailureDetected() async {
        ...
        for _ in 0..<3 {
            await detector.record(ShikkiEvent(
...
struct EventRouterPipelineTests {
    ...
    func fullPipeline() async {
        ...
        let event = ShikkiEvent(source: .orchestrator, type: .sessionStart, scope: .session(id: "test"))
