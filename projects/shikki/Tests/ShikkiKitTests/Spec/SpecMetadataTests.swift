...
struct SpecLifecycleStatusTests {
    ...
    @Test("Review can transition back to draft")
    func reviewToDraft() {
        #expect(SpecLifecycleStatus.review.canTransition(to: .draft))
    }
    ...
    func draftCannotSkipToValidated() {
        #expect(!SpecLifecycleStatus.draft.canTransition(to: .validated))
    }
    ...
    @Test("Rejected can transition to draft or outdated")
    func rejectedTransitions() {
        #expect(SpecLifecycleStatus.rejected.canTransition(to: .draft))
        #expect(SpecLifecycleStatus.rejected.canTransition(to: .outdated))
    }
}
...
struct SpecMetadataModelTests {
    ...
    func reviewerVerdictRawValues() {
        let rawValues = SpecReviewerVerdict.allCases.map(\.rawValue)
