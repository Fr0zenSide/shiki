...
struct GitProjectStatusTests {
    ...
    @Test("Git format with status shows counts")
    func gitFormatBranch() {
        let info = GitStatusFormatter.GitInfo(branch: "feature/shiki-ship", staged: 1, modified: 2)
        let output = GitStatusFormatter.format(info)
        // Branch is not shown in tmux segment (shown in terminal prompt instead)
        // but status counts should appear
        #expect(output.contains("+1"))
        #expect(output.contains("!2"))
    }

    @Test("Git format abbreviates branch via abbreviateBranch helper")
    func gitFormatLongBranch() {
        // The format() method doesn't include branch in output (it's for tmux),
        // but we can verify it produces valid output with worktree/status info
        let info = GitStatusFormatter.GitInfo(
    ...
    @Test("Git format clean repo produces empty output")
    func gitFormatClean() {
        // A clean repo with no remote produces no tmux segment content
        // (branch is shown in terminal prompt, not tmux status bar)
