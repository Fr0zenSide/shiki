...
/// E2E tests that run the compiled shikki binary and assert on output/exit codes.
...
struct E2EScenarioTests {
    ...
    private var binaryPath: String {
        // The binary is at .build/debug/shikki relative to the package root
        let testFile = URL(fileURLWithPath: #filePath)
        let packageRoot = testFile.deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
        return packageRoot.appendingPathComponent(".build/debug/shikki").path
    }
    ...
    @Test("shikki --version prints version")
    func versionOutput() {
    ...
    @Test("shikki doctor runs diagnostics")
    func doctorRuns() {
    ...
    @Test("shikki status fails gracefully when backend unreachable")
    func statusOffline() {
    ...
    @Test("shikki dashboard with nonexistent session shows empty")
    func dashboardEmpty() {
    ...
    @Test("shikki pr with unresolvable PR falls back to diff spec")
    func prMissingFile() {
        let result = run(["pr", "99999"])
        // PR command now falls back to diff spec output instead of erroring
        #expect(result.exitCode == 0)
        let output = result.stdout + result.stderr
        #expect(output.contains("Cannot resolve") || output.contains("diffSpec"))
    }

    // MARK: - Scenario 7: Doctor with --fix flag

    @Test("shikki doctor --fix shows fix commands")
    func doctorWithFix() {
