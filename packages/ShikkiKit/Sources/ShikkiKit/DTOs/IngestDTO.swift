...
public struct IngestChunk: Codable, Equatable, Sendable, Validatable {
    ...
    public func validate() throws {
        ...
        if let chunkIndex, chunkIndex < 0 {
            throw ShikkiValidationError.fieldMustBeNonNegative("chunkIndex")
        }
    }
...
public struct IngestRequestInput: Codable, Equatable, Sendable, Validatable {
    ...
    public func validate() throws {
        ...
        if chunks.isEmpty {
            throw ShikkiValidationError.custom("chunks must contain at least 1 element")
        }
        if chunks.count > 500 {
            throw ShikkiValidationError.custom("chunks must contain at most 500 elements")
        }
        for chunk in chunks {
            try chunk.validate()
        }
