...
/// Persists lifecycle events to ShikkiDB via HTTP POST.
...
public actor EventPersister {
    let dbURL: String
    private let logger = Logger(label: "shikki.core.event-persister")
    ...
    /// Persist an event to ShikkiDB. Never throws — logs warning on failure.
    public func persist(_ event: LifecycleEventPayload) async {
        do {
            ...
            {
                logger.warning(
                    "ShikkiDB returned non-success status",
                    metadata: [
            ...
            logger.warning(
                "Failed to persist event to ShikkiDB — continuing",
                metadata: [
