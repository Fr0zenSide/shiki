...
public protocol Validatable: Sendable {
    /// Validates the input, throwing `ShikiError.validationFailed` on failure.
...
public enum Validators {
    public static func requireNonEmpty(_ value: String, field: String) throws {
        if value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            throw ShikiValidationError.fieldEmpty(field)
        }
    }

    public static func requirePositive(_ value: Int, field: String) throws {
        if value < 1 {
            throw ShikiValidationError.fieldMustBePositive(field)
        }
    }

    public static func requireNonNegative(_ value: Int, field: String) throws {
        if value < 0 {
            throw ShikiValidationError.fieldMustBeNonNegative(field)
        }
    }

    public static func requireRange(_ value: Double, min: Double, max: Double, field: String) throws {
        if value < min || value > max {
            throw ShikiValidationError.fieldOutOfRange(field, min: min, max: max)
        }
    }

    public static func requireValidUUID(_ string: String, field: String) throws {
        if UUID(uuidString: string) == nil {
            throw ShikiValidationError.invalidUUID(field)
        }
    }
}

/// Validation errors before they are wrapped into ShikiError.
public enum ShikiValidationError: Error, Equatable, Sendable {
